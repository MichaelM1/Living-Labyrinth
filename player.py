...
class Player(object):
    ...
    def checkForCollision(self, dx ,dy):
        for wall in self.data.walls:
            if self.x > (wall.x + HITBOX_LEEWAY + WALL_SIZE - (HITBOX_LEEWAY * 2)) or wall.x + HITBOX_LEEWAY > (self.x + 16):
                self.collision = False
            elif self.y > (wall.y + HITBOX_LEEWAY + WALL_SIZE - (HITBOX_LEEWAY * 2)) or wall.y + HITBOX_LEEWAY > (self.y + 16):
                self.collision = False
            else: 
                self.collision = True
                break
